...
from HR.models import LeaveDetails,EmpDesk,ProjectDetails,Manager,Project_team,Attendance
    ...
    if request.session['user']=="2":
        ...
        if request.method=='POST':
            if 'emp_details' in request.POST:
                ...
                return redirect('../clientmanager/CM_Employee_Details')
        ...
        empl = EmpDesk.objects.filter(emp_type='3', manager_id=request.session['id'],activation='Active')
        ...
        request.session['start_date']=str(start_date)
        request.session['end_date']=str(end_date)
        approved=Empdata.objects.filter(status=1, manager_id=request.session['id']).values('emp_id', 'start').distinct()
        total_approved=len(approved)
        pending=Empdata.objects.filter(status=4, manager_id=request.session['id']).values('emp_id', 'start').distinct()
        ...
        approved_last = Empdata.objects.annotate(start_month=Cast('start', output_field=DateField())).filter(status=1,manager_id=request.session['id'],start_month__year=year,start_month__month=last_month).values('emp_id', 'start').annotate(total_approved_last=Count('id')).distinct()

        pending_last = Empdata.objects.annotate(start_month=Cast('start', output_field=DateField())).filter(status=4,manager_id=request.session['id'],start_month__year=year,start_month__month=last_month).values('emp_id', 'start').annotate(total_pending_last=Count('id')).distinct()
        ...
        noti = zip(noti,image)
        return render(request, 'CM_dashboard.html', {'name': name,'noti':noti,'notify':notify,'total_approved_last':total_approved_last,'total_pending_last':total_pending_last, 'employee':empl,'project':projects,'total_pending':total_pending,'total_approved':total_approved})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_approvedTimesheet(request):
    ...
    if request.session['user']=="2":
        ...
        if request.method=="POST" :
            ...
            if 'prev' in request.POST:
                ...
                return redirect('../clientmanager/CM_empTracker_Details')
                
        hr_id=Manager.objects.get(emp_id=request.session['id']).hr_id
        start_dts=start_date.strftime("%Y-%m-%d ")        
        task = Empdata.objects.filter(start=start_date)
        cursor = connection.cursor()
        print(f" hr_pending_timesheet '{start_date}','1','1','{hr_id}','{request.session['id']}'")
        cursor.execute(f" hr_pending_timesheet '{start_date}','1','1','{hr_id}','{request.session['id']}'")    
        if request.session['work_location'] == 'US':
            cursor.execute(f" hr_pending_timesheet_us '{start_date}','1','1','{hr_id}','{request.session['id']}'") 
        ...
        return render(request, 'CM_approvedTimesheet.html', {'pending':pending,"task": task, "name": name, 'i': task, 'start': start_date, 'end': end_date})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_leave(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        sl=[]
        pl=[]
        leaves_detail=EmpLeaves.objects.filter(user_type='3',status='Approved',manager_id=manager_id).order_by('-id')
        if request.session['work_location']=='US':
            leaves_detail=EmpLeaves.objects.filter(user_type='3',manager_id=manager_id).order_by('-id')
        ...
        return render(request, 'CM_leave.html', {'emp_leaves':emp_leaves,'manager_sl':manager_sl, 'manager_pl':manager_pl,'name': name,'leaves_detail':leaves,'remaining':leaves,'taken':leaves})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_EditProfile(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        name = request.session['name']
        return render(request, 'CM_EditProfile.html', {'name': name,'cm':cmPro, 'message':message,'message1':message1})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_notification(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        notif = Notifications.objects.filter(manager='1',date=start_date,manager_id=request.session['id'])
        name = request.session['name']
        return render(request, 'CM_notification.html', {'name': name,'notif':notif})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_pendingTimesheet(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        if request.method=="POST" and ('prev' in request.POST or 'next' in request.POST):
            ...
            if 'pending' in request.POST:
                ...
                return render(request, 'CM_empTracker.html',{'comments':comments,'img':img,'name': name,'designation':designation,'tue_comm':tue_comm,'wed_comm':wed_comm,'thu_comm':thu_comm,'fri_comm':fri_comm,'sat_comm':sat_comm,'data':data,'date':starts,'emp_id':emp_id,'count': count, 'start_date': start_date, 'end_date': end_date ,'emp_name':emp_name ,"employee_id":employee_id, 'employee_start':employee_start,'employee_end':employee_end,'tasks':task })
                ...
                obj=Notifications.objects.filter(emp_id=j.emp_id,message=f'Your Timesheet for Week ({j.start} - {j.end}) is Rejected by the Manager Today ({datetime.now().date()}) ',manager_id=request.session['id'])  
                if len(obj)>0:
                    obj.delete()
                notify=Notifications(emp_id=j.emp_id,emp=1,hr=0,manager=0, message=f'Your Timesheet for Week ({j.start} - {j.end}) is Rejected by the Manager Today ({datetime.now().date()}) ',emp_name=j.name,date=start_date,current_date=datetime.now().date(),manager_id=request.session['id'])
                notify.save()
                hr_mail=Login.objects.get(id=hr_id).email
                ...
                try:
                    ...
                    new_rating.save()
                Empdata.objects.filter(emp_id=emp_id,start=str(start_date)).update(status=1)
                ...
                #     i.save(update_fields=['status'])
                obj=Notifications.objects.filter(emp_id=j.emp_id,message=f'Your Timesheet for Week ({j.start} - {j.end}) is Approved by the Manager Today ({datetime.now().date()}) ',manager_id=request.session['id'])  
                # if len(obj)>0:
                #     obj.delete()
                ...
                Your Timesheet for period {start_date} to {end_date}  is Approved by Manager 
        ...
        cursor = connection.cursor()
        print(f"------------------------------- hr_pending_timesheet '{start_date}','4','0','{hr_id}','{request.session['id']}'----------------------------------")
        
        cursor.execute(f" hr_pending_timesheet '{start_date}','4','1','{hr_id}','{request.session['id']}'") 
        if Login.objects.get(id=request.session['id']).work_location == 'US':
            cursor.execute(f" hr_pending_timesheet_us '{start_date}','4','1','{hr_id}','{request.session['id']}'")
        ...
        start_date=str(start_date)
        end_date=str(end_date)
        return render(request, 'CM_pendingTimesheet.html', {'pending':pending,"task": task, "name": name, 'obj': obj, 'count': count, 'start_date': start_date, 'end_date': end_date})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_profile(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        try:
            ...
            pass
        return render(request, 'CM_profile.html', {'name': name,'profile':profileData})
    else:
        response=HttpResponseRedirect('../../')
        return response

def CM_empTracker(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        name = request.session['name']
        return render(request, 'CM_empTracker.html', {'name': name})
    else:
        response=HttpResponseRedirect('../../')
        return response
    
def CM_empTracker_Details(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        try:
            ...
            fri_comm = ''
            sat_comm = ''
        return render(request, 'CM_empTracker_Details.html',{'comments':comments,'data':data,'empdesk':empdesk,'img':img,'tasks':task,
    ...
    if request.session['user']=="2":
        ...
        if request.method=='POST':
            ...
            if 'addEmp' in request.POST:
                ...
                print(project,'vlogs')
                manager_selected=str(Login.objects.get(id=request.session['id']).f_name) +' '+ str(Login.objects.get(id=request.session['id']).l_name)
                teams=ProjectDetails.objects.filter(project_name=project)
                team='Team'+str(teams[0].id)

                emp_id=employee_name
                try:
                    emp_id=EmpDesk.objects.get(email=emp_id).login_id
                except:
                    pass
                ...
                print(project,  employee_name, designations)
                ...
                response=HttpResponseRedirect('/clientmanager/CM_phase')
                return response
                  
        managers=Manager.objects.all()
        all_projects=ProjectDetails.objects.filter(manager_id=request.session['id'])

        projects=ProjectDetails.objects.filter(manager_id=request.session['id'])
        manager_name= Manager.objects.get(emp_id=request.session['id']).name  
        team=Project_team.objects.filter(manager=manager_name)
        name=request.session['name']
        employees=EmpDesk.objects.filter(emp_type='3',activation='Active',manager_id=request.session['id'])
        project=[]
        pending=[]
        development=[]
        completed=[]
        ...
        return render(request,'CM_team.html',{'all_projects':all_projects,'name':name,'team':team,'employees':employees,'managers':managers,"projects":projects})
    else:
        response=HttpResponseRedirect('../../')
        return response
    
def CM_projects(request):
    try:
        if request.session['user']=="2":
            pass
        else:
    ...
    if request.session['user']=="2":
        ...
        project=ProjectDetails.objects.filter(manager_id=request.session['id'])   
        name=request.session['name']
...
def CM_Project_task(request):
    ...
    return render(request, "CM_Project_task.html" ,{'project_team_task':project_team_task,'employees':employees,'project_team':project_team, 'tasks_list':tasks_list, 'development':development, 'completed':completed,'chats':chats})
...
def Admin_Employee_Details(request):
    ...
    image_data = Login.objects.filter(id=request.session['emp_details']).values('image').first()
    ...
    return render(request, 'CM_EmpDetails.html',{'month':month,'image_data':image_data,'year':year,'empdetails':empdetails,'weekly_data':weekly_data})

def CM_phase(request):
    message=''
    ...
    for h in versions:
        ...
        for i in  phase:
            ...
            for j in task:
                ...
                for cr in crew:
                    if EmpDesk.objects.filter(email=cr.employee_name ).first().activation=='Active':
    ...
    return render(request,"CM_phase.html",{'current_phase':current_phase,'message':message,'version':version,'employees':employees,'project_team_task':project_team_task,'current_version':current_version})
